...
- **The Ritual Story (`app/meet/story/`).** An in-call explainer Samuel drives from his copilot to offload the long Phase-9 roadmap monologue onto the prospect's screen. Renders BELOW `<VariablesPanel>` in the same `.demo-call-room-notes` aside — one continuous column that "continues the notes like a story." **Redesigned 2026-05-30** (model corrected after reviewing the ritual/onboarding/script + landing-page + negotiation skills). The **document is a persistent spine** (`doc-spine.tsx`), rendered above every beat: a page card whose first section ("Problema y Solución") is seeded LIVE from the prospect's `behaviour_to_change` / `core_motivation`, with the other 8 sections ghosted "to-come" (`locked`) + a `1 / 9` progress meter — the curiosity engine (he can see how much of his own doc is still his to write). `enemy_name` only ever appears as a ghosted slot — it's captured in onboarding, not the demo. Three beats Samuel advances over the spine: `promise` (the reworked neuro — keeps the old-pattern-vs-ritual base and LAYERS the two changes: behaviour rises fast, thoughts & feelings rise slowly; three curves, copy opens with the tactical-empathy "you don't have to feel different first") → `experience` (the six steps = the whole multi-session journey: map → design → live → **optimize** → live → repeat — distinct from the daily loop) → `loop` (NEW daily-loop beat: agent calls → you do the ritual → a short call tracks it; generic, does NOT name the tracking agent). Files: `ritual-story.tsx` (persistent `<DocSpine>` + `AnimatePresence` beat switch + the `StoryStage` union mirror), `doc-spine.tsx`, `neuro-crossfade.tsx` (exports `PromiseBeat`), `cycle-map.tsx` (`CycleMap`), `daily-loop.tsx` (`DailyLoop`), `strings.ts` (bilingual EN/ES voseo — copy is editorial, NOT sloganeering; preserves the original approved neuro/doc voice), `story.css` (reuses `call.css`'s `:root` tokens, literal Fraunces/Manrope). `StoryStage = "hidden" | "promise" | "experience" | "loop"` — the doc spine is NOT a stage; `stage="hidden"` (default) renders nothing at all. Transport: the `demo-call:show_visual` event on the SAME DataChannel (carries only `{ stage }`); `call-room.tsx`'s `onDataMessage` branches on `type`, validates the stage, sets `storyStage`. Personalization is free — the story reads the same `variables` state the panel already holds. Reduced-motion via `useReducedMotion()`. The rep-side broadcaster + sticky `<StoryControl>` (now Promise/Experience/Daily Loop) live in samwise-app. Phase-9 SAY-block cut in the Demo script is a SEPARATE gated step.

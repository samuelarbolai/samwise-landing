...
## `/demo-call/[bookingId]` (first-class route, not a variant)

User-side surface for the human-to-human Demo Call with Samuel. The therapist side lives in `samwise-app` at `app.samwise.life/demo-call/[bookingId]`; this is the prospect's view at `samwise.life/demo-call/[bookingId]`. Both sides resolve to the same LiveKit room via `samwise-backend/cloud-functions`'s `demoBookings/{calBookingUid}` doc, written by the `calDemoBookingWebhook` cloud function when Cal.com fires `BOOKING_CREATED` for the `demo` event type.

- **Route structure** (`app/demo-call/[bookingId]/`): `page.tsx` (server, thin) → `call-root.tsx` (client orchestrator) → `pre-join.tsx` (lobby) OR `call-room.tsx` (in-call). `video-call-experience.tsx` is the canonical client wiring for a two-human LiveKit room — **a verbatim duplicate** of `samwise-app/components/demo-call/VideoCallExperience.tsx`. Keep the two in lockstep; extract to a shared package if the maintenance friction surfaces.
- **Pre-join lobby.** POSTs `bookingId` + `side: "user"` cross-origin to `${NEXT_PUBLIC_SAMWISE_APP_URL}/api/demo-call/init`. On success: Fraunces italic lead + Manrope small-caps sub + hairline gold-dash CTA (matches `/qualify` register). On 404/410: warm explanation + back-to-Samwise link. The `Test mic and camera` button surfaces the browser permission prompt before commitment.
- **No language picker.** Language is determined by the booking (Cal.com form field, stored on `demoBookings`). Strings live in `lib/demo-call/strings.ts`, keyed by `Lang` from `lib/qualify/strings.ts`.
- **No "demo" in user-facing copy.** The URL path keeps the internal name for routing; copy says "your call with Samuel" / "your session" — same rule that landed *"Book the call →"* on the qualify FinalScreen.
- **In-call layout.** Two regions: video tile (main, 1.8fr) + variables panel (right rail, 1fr, min 280px). Mobile (below 900px): video stacks above notes. The variables panel is the existing `<VariablesPanel>` from `/qualify/components/`, hydrated from `demo-call:variable_update` LiveKit DataChannel events published by the therapist's copilot when a `userVisible: true` variable's cleaned value changes. Client-side `ALLOWED_KEYS` set is a defensive filter in case the broadcaster ever sends a non-user-visible key.
- **Video tile internals.** `VideoCallExperience` publishes camera + mic together via `createLocalTracks`, attaches remote video to `<video autoPlay playsInline>` elements (filling the tile via CSS `position: absolute; inset: 0; object-fit: cover`), and renders a self-view PiP (168×120 top-right). Mic is open by default with a Mute toggle (NOT push-to-talk — this is human-to-human, not human-to-agent). Tab-hidden auto-mute mirrors `/qualify`'s privacy contract. A 75-minute wall-clock hard cap force-disconnects (45-min slot + 30-min overrun buffer); LiveKit's `emptyTimeout` (~30s) closes the room when both sides have left.
- **CSS conventions.** `demo-call.css` redeclares brand tokens locally (`--bg`, `--ink`, `--gold #D4A85A`) and uses literal `'Fraunces', Georgia, serif` / `'Manrope', sans-serif` stacks. **Do NOT use `var(--font-fraunces)`** — the variable doesn't exist in this codebase and resolves to empty (caught by `preview_inspect` during qualify's finalize-indicator build; same rule applies here). The CSS also re-declares the minimum `.qualify-notes-*` rules so the reused `<VariablesPanel>` renders correctly without dragging in the whole `qualify.css`.
- **Env vars on Vercel for `/demo-call`:** `NEXT_PUBLIC_SAMWISE_APP_URL` (e.g. `https://app.samwise.life` in prod, `http://localhost:3000` in dev) — drives the cross-origin init POST. LiveKit env vars are NOT needed on samwise-landing for this route because token minting happens server-side on samwise-app; the landing only consumes the token via the init response.

## `/book` (Samwise-branded Cal embed)

Booking surface for the Demo Call. Samuel sends qualified prospects this URL (instead of the raw cal.com link) after the Breakthrough Call. The Cal element-click modal opens in place when the CTA is tapped — Cal's own UI handles the calendar pick + confirmation; once a slot is booked, samwise-backend's `calDemoBookingWebhook` cloud function writes the `demoBookings/{calBookingUid}` doc and the prospect receives the `samwise.life/demo-call/{{uid}}` link in their inbox.

- **Bilingual via `?lang=es` query param** (default `en`). No picker — Samuel sends the language-correct URL. Strings live inline in `book-client.tsx`.
- **Cal embed init.** `useEffect` injects the canonical `embed.js` snippet from Cal admin, then `Cal("init", "demo", { origin: "https://app.cal.com" })` + `Cal.ns.demo("ui", { layout: "month_view" })`. The CTA button carries `data-cal-link="samuel-giraldo-concha-yqvtot/demo"` + `data-cal-namespace="demo"` so the embed.js attaches its click handler globally.
- **Aesthetic.** Mirrors `/qualify`'s picker register: gallery white, Fraunces italic lead, Manrope small-caps sub, hairline gold-dash CTA. Brand tokens declared locally in `book.css` with literal `'Fraunces' / 'Manrope'` stacks (no `var(--font-fraunces)`).
- **No "demo" in user-facing copy.** CTA reads "Open the calendar" / "Abrir el calendario." Lead/sub do the work without naming the session type — same restraint that landed on the `/qualify` FinalScreen.
- **Email follow-up.** If Cal substitutes `{{uid}}` in the location URL field on Samuel's account, the confirmation email carries the join link natively. If not, the fallback is to extend `calDemoBookingWebhook` to write a `mail/{auto-id}` doc with the join URL (Firebase Trigger Email is already wired). See `current-plan.md` Step 10.4.

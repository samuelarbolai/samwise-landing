...
- **The Ritual Story (`app/meet/story/`).** An in-call explainer Samuel drives from his copilot to offload the long Phase-9 roadmap monologue onto the prospect's screen. Renders BELOW `<VariablesPanel>` in the same `.demo-call-room-notes` aside — one continuous column that "continues the notes like a story." Three scenes advanced by Samuel: `doc` (the Ritual Doc as a 3-section page card, the prospect's captured `behaviour_to_change` / `core_motivation` slotted into the active "Problem & Solution" section) → `cycle` (six steps headlined by their gold OUTPUT into the doc, doc pinned on top as the spine) → `neuro` (two SVG curves: the old pattern descends, the ritual rises, they cross; descending curve labeled with the prospect's own truncated behaviour). Files: `ritual-story.tsx` (AnimatePresence `mode="wait"` stage switch + the `StoryStage` union mirror), `doc-spine.tsx`, `cycle-map.tsx`, `neuro-crossfade.tsx`, `strings.ts` (bilingual EN/ES, voseo), `story.css` (reuses `call.css`'s `:root` tokens, literal Fraunces/Manrope stacks). Transport: a NEW `demo-call:show_visual` event on the SAME DataChannel (carries only `{ stage }`); `call-room.tsx`'s `onDataMessage` now branches on `type` and sets `storyStage`. Personalization is free — the story reads the same `variables` state the panel already holds, so the event re-sends no data. `stage="hidden"` (default) renders nothing. Reduced-motion via `useReducedMotion()` (static fallbacks). The rep-side broadcaster + `<StoryControl>` panel live in samwise-app. Phase-9 SAY-block cut in the Demo script is a SEPARATE gated step, done only after the visual is proven on a real call.

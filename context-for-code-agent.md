...
- **New reusable components** (built self-contained so the FUTURE therapist-Demo-Call surface can recycle them, mirroring how `demo-voice-room.tsx` recycled `/qualify`): `seven-steps.tsx` (the 7 steps the therapist commits to, verbatim from the offer), `offer-card.tsx` (the user's verbatim ask/offer — 50% / $25-of-$50 + the "yet to be proven" caveat), `collaboration.tsx` (where the therapist plugs into onboarding / call design / optimization), `personalization-capture.tsx` (the close).
- **The case (Sarah)** lives in `case-data.ts` — sourced from her real ritual doc, PII STRIPPED (first name only, no phone). Motivation + the disidentification mantra translated FULLY to English. Kept SIMPLE: the desidentification turn (she opened up + adopted the ritual once she stopped identifying with the problem), her ritual (protection / new belief), and her three daily calls (Morning Protection / Afternoon Faith-Building / Evening Prep — these feed the `DailyLoop` beat's "frequency & type of agent calls"). `SARAH_VARS` (behaviour_to_change / core_motivation / etc.) feeds `DocSpine` slots + `PromiseBeat`'s descending-curve label.
- **The close = on-the-spot personalization capture** (`personalization-capture.tsx`), FULLY FUNCTIONAL frontend, NO backend / NO email. Three process paths (use template as-is / bring your own process / edit the script here) + the personalization fields (you, region/tz, languages, price, pace, behaviours, first user, per-step fit signal, current note tools, default cadence, revenue-model checkbox). On "Assemble" it builds a plain-text summary shown on screen + a copy-to-clipboard button (Samuel grabs it live). Below it, a quiet `/book` link: "Book a quick 15-minute test of adopting Samwise" (reuses the Breakthrough Call picker for v1). **SAFETY: never collect payment credentials here** — email + free-text only. Backend personalization is manual/later (Samuel's decision).
